...
export const AI_NAMES = ['Marcus', 'Sarah', 'David', 'Elena', 'James', 'Kaito', 'Luna', 'Viktor', 'Oksana', 'Felix'];
...
export const initializeGame = (config: GameConfig = DEFAULT_CONFIG): GameState => {
    ...
    for(let i=0; i<config.opponentCount; i++) {
        ...
        const variance = Math.floor(Math.random() * (config.startingStackAI * 0.2)) - (config.startingStackAI * 0.1);
