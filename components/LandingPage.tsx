...
export const LandingPage: React.FC<LandingPageProps> = ({ onStartGame, username, isExiting }) => {
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[0.6rem] md:text-xs font-mono text-white/50 uppercase tracking-widest">
                                    PLAYER: <span className="text-white">{username || 'UNKNOWN'}</span>
                                </span>
                            </div>
                        </div>
                    </div>
                                        ...
                                        >
                                            <div className="flex justify-between items-start w-full">
                                                <div className="flex flex-col">
                                                    <span className={`text-[0.6rem] font-mono uppercase tracking-widest mb-1 ${isActive ? 'text-black/60' : 'text-gray-500'}`}>
                                                        {level.sub}
                                                    </span>
                                                    <span className={`text-lg font-bold font-sans tracking-tight leading-none ${isActive ? 'text-black' : 'text-white'}`}>
                                                        {level.name}
                                                    </span>
                                                </div>
                                                {isActive && (
                                            ...
                                            <div className="mt-auto flex flex-col gap-1">
                                                <div className="flex justify-between items-end border-b border-black/10 pb-2 mb-2">
                                                    <span className="text-[0.6rem] uppercase font-bold">Buy-In</span>
                                                    <span className="font-mono text-xl font-bold tracking-tighter">
                                                        ${level.buyIn.toLocaleString()}
                                                    </span>
                                                </div>
                                                <div className="flex justify-between items-end">
                                                    <span className="text-[0.6rem] uppercase font-bold">Blinds</span>
                                            ...
                                            <div className={`absolute -bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[0.5rem] uppercase tracking-widest font-bold shadow-sm ${isActive ? 'bg-black text-[#d4af37]' : 'bg-[#222] text-gray-500'}`}>
                                                {level.desc}
                                            </div>
                                        </button>
                                    );
                    ...
                    <div className="bg-black/20 border border-white/5 rounded-2xl p-6 backdrop-blur-sm">
                        ...
                        <div className="grid grid-cols-3 gap-3 md:gap-4">
                            {[
                                ...
                                { total: 10, opponents: 9, label: 'PLATOON', desc: 'Full Ring' }
                            ].map((size) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className="flex items-baseline gap-1 mb-1">
                                            <span className={`text-2xl md:text-3xl font-mono font-bold tracking-tighter ${isSelected ? 'text-[#d4af37]' : 'text-white/40 group-hover:text-white/60'}`}>
                                                {size.total}
                                            </span>
                                            <span className={`text-[0.6rem] font-bold uppercase ${isSelected ? 'text-[#d4af37]/80' : 'text-white/20'}`}>
                                                PLAYER
                                            </span>
                                        </div>
                                        
                                        <span className={`text-[0.55rem] uppercase tracking-[0.2em] font-bold mb-1 ${isSelected ? 'text-white' : 'text-white/30'}`}>
                                            {size.label}
                                        </span>
                                        
                                        <span className={`text-[0.5rem] font-mono ${isSelected ? 'text-[#d4af37]/60' : 'text-white/20'}`}>
                                            {size.desc}
                                        </span>
