...
const evaluateHand = (cards: Card[]): HandRank => {
    ...
    let bestHand: HandRank = { rankValue: -1, tieBreakers: [], name: '', winningCardIds: [] };
...
const evaluate5CardHand = (hand: Card[]): HandRank => {
    ...
    if (countValues.includes(4)) {
        ...
        return { rankValue: 7, tieBreakers: [quadRank, kicker], name: 'Four of a Kind', winningCardIds: cardIds };
    }
    if (countValues.includes(3) && countValues.includes(2)) {
        ...
        return { rankValue: 6, tieBreakers: [trips, pair], name: 'Full House', winningCardIds: cardIds };
    }
    if (isFlush) {
        if (isStraight) {
            const highCard = ranks[0] === 14 && ranks[1] === 5 ? 5 : ranks[0];
            return { rankValue: 8, tieBreakers: [highCard], name: 'Straight Flush', winningCardIds: cardIds };
        }
        return { rankValue: 5, tieBreakers: ranks, name: 'Flush', winningCardIds: cardIds };
    }
    if (isStraight) {
        const highCard = ranks[0] === 14 && ranks[1] === 5 ? 5 : ranks[0];
        return { rankValue: 4, tieBreakers: [highCard], name: 'Straight', winningCardIds: cardIds };
    }
    if (countValues.includes(3)) {
        const trips = countKeys.find(k => counts[k] === 3)!;
        const kickers = countKeys.filter(k => k !== trips).sort((a,b) => b-a);
        return { rankValue: 3, tieBreakers: [trips, ...kickers], name: 'Three of a Kind', winningCardIds: cardIds };
    }
    if (countValues.filter(c => c === 2).length === 2) {
        const pairs = countKeys.filter(k => counts[k] === 2).sort((a,b) => b-a);
        const kicker = countKeys.find(k => counts[k] === 1)!;
        return { rankValue: 2, tieBreakers: [...pairs, kicker], name: 'Two Pair', winningCardIds: cardIds };
    }
    if (countValues.includes(2)) {
        const pair = countKeys.find(k => counts[k] === 2)!;
        const kickers = countKeys.filter(k => k !== pair).sort((a,b) => b-a);
        return { rankValue: 1, tieBreakers: [pair, ...kickers], name: 'Pair', winningCardIds: cardIds };
    }
    return { rankValue: 0, tieBreakers: ranks, name: 'High Card', winningCardIds: cardIds };
};

/**
 * Calculates winners for ALL pots (Main + Sides).
 */
export const determineWinner = (players: Player[], board: Card[], pots: Pot[]): WinnerResult => {
    ...
    pots.forEach((pot, index) => {
        ...
        const ties = eligibleContenders.filter(c => compareHandRanks(c.handRank, winner.handRank) === 0);
    ...
    if (mainPotPayout) {
        ...
        payouts.forEach(p => {
            winningsMap.set(p.playerId, (winningsMap.get(p.playerId) || 0) + p.amount);
        });
        const sortedWinners = Array.from(winningsMap.entries()).sort((a, b) => b[1] - a[1]);
        primaryWinnerId = sortedWinners[0][0];
    } else {
         return {
            primaryWinnerId: '',
            primaryHand: { rankValue: -1, tieBreakers: [], name: 'Unknown', winningCardIds: [] },
            payouts: [],
            isSplit: false
        };
    }
