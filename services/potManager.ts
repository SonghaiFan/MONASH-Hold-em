...
export const resolvePots = (players: Player[], currentPots: Pot[]): Pot[] => {
  ...
  while (activeBets.some((p) => p.bet > processedBetAmount)) {
    ...
    const remainingContributors = activeBets.filter(
      (p) => p.bet > processedBetAmount
    );
    ...
    const eligibleIds = contributors.filter((c) => c.isActive).map((c) => c.id);
    ...
    if (
      lastPot &&
      areArraysEqualUnordered(lastPot.eligiblePlayerIds, eligibleIds)
    ) {
